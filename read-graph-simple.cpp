...
Graph read_graph(std::string filename) // JE MODIFIE LA FONCTION POUR QU'ELLE RENVOIE UN GRAPH
{
    Graph gr;
    ...
    if (!input_file.is_open())
    {
        ...
        return gr; // on termine la fonction
    }
    ...
    for (int i{0}; i < nb_vertices; i++) // JE CREE TOUS LES SOMMETS NECESSAIRES
    {
        Vertex vertex(i);
    ...
    while (getline(input_file, line))
    {
        ...
        gr.add_edge(from, to, value); // ON MET LA TRANSITION DANS LE GRAPHE

        // std::cout << "transition " << from << " -(" << value << ")-> " << to << std::endl;
        // PLUS BESOIN DE CETTE LIGNE SI ON NE VEUT QUE CREER LE GRAPHE
    }
    return gr;
}
